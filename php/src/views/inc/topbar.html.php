...
<header class="h-14 flex items-center gap-3 px-4 border-b border-border bg-background flex-shrink-0">
  ...
  <span class="hidden sm:block text-foreground font-semibold" style="font-size:14px;"> <?= Session::isConnected() ? htmlspecialchars(Session::getCurrentUser()->getNom()) : '' ?> </span>
  ...
  <div class="flex-1 flex items-center gap-2 max-w-sm mx-auto">
    <div class="relative flex-1">
      ...
      <form action="<?= '/' . ($page ?? 'index') ?>" method="GET" class="m-0">
        <input type="text" name="q" value="<?= htmlspecialchars($search ?? '') ?>" placeholder="Rechercher ressources, groupes…"
