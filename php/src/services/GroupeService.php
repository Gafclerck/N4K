...
class GroupeService
{
    ...
    public function hydrateGroupes(array $groupes): array
    {
        ...
        if (Session::isConnected()) {
            ...
            if ($user) {
                $memberships = $membreRepo->selectByUser($user->getId());
                $myGroupIds = array_map(fn($m) => $m->getGroupeId(), $memberships);
            }
        }
        foreach ($groupes as $g) {
            $g->setIsMember(in_array($g->getId(), $myGroupIds));
            $g->setNbrMembres($this->groupeRepo->countMembers($g->getId()));
        }
        return $groupes;
    }
